...
def main():
    ...
    if st.button("Analyze"):
        ...
        if (sentiment==0):
            # Check for and remove specific words related to class
            words_to_remove = ['sarcasm', 'sarcastic', 'irony', 'ironic']
            text_words = text.split()
            text = ' '.join([word for word in text_words if word not in words_to_remove])
            pred="figurative"
        elif(sentiment==1):
            pred="sarcasm"
        elif (sentiment==2):
